...
pub fn warm_up_index(index: &SpecialistIndex) {
...
pub fn warm_up_payload_path(index: &SpecialistIndex) {
...
fn warm_up_payload_body(
    ...
    if let Some((req, _)) = http::parse_request(request) {
        let _ = super::handler::handle_request(&req, index, ready);
    }
}

pub fn warm_up_via_http(url: &str, duration_ms: u64, concurrency: usize, payloads_path: &str) {
